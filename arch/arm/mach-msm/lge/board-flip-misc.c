...
//    static int same_count;
    ...
    // C729 RevB RevC RevD vibrator is not working if exists PWM
    if(lge_get_hw_rev() <= LGE_REV_D) return 0;
...
*/
	...
	if(gp_clk == NULL)
	{
		printk("----------GP_clk is NULL --- clk_get(NULL,gp_clk)---------\n");
...
static struct lge_vibrator_platform_data flip_vibrator_data = {
	.enable_status = 0,
...
//	.amp_value = 105,
 ...
 static struct platform_device android_vibrator_device = {
 	...
 	.id = -1,
 	.dev = {
 		.platform_data = &flip_vibrator_data,
 	},
 };
