...
static unsigned short atcmd_virtual_keycode[ATCMD_VIRTUAL_KEYPAD_ROW][ATCMD_VIRTUAL_KEYPAD_COL] = {
	{KEY_1, 		KEY_8, 				KEY_Q,  	 KEY_I,          KEY_D,      	KEY_HOMEPAGE,	KEY_B,          KEY_UP},
...
static struct regulator_bulk_data vreg_kt_set[] =
{
		{ .supply = "lvsw1"},
};

static struct regulator *vreg_kt;
void __init key_touch_power_init(void)
{	
	int rc;

	gpio_tlmm_config(key_touch_ldo_config,	GPIO_CFG_ENABLE);

	rc = regulator_bulk_get(NULL, ARRAY_SIZE(vreg_kt_set), vreg_kt_set);
	if (rc) {
		pr_err("%s: could not get regulators: %d\n", __func__, rc);
		goto out;
	}

	rc = regulator_bulk_set_voltage(ARRAY_SIZE(vreg_kt_set), vreg_kt_set);
	if (rc) {
	...
	vreg_kt = vreg_kt_set[0].consumer;

	return;
	reg_free:
		regulator_bulk_free(ARRAY_SIZE(vreg_kt_set), vreg_kt_set);
...
{
#if 0 /* LGE_CHANGE_S 20120206, taebum81.kim@lge.com */
	struct vreg *vreg_kt;
#else
//	struct regulator *vreg_kt;
#endif /* LGE_CHANGE_E 20120206, taebum81.kim@lge.com */

	/* LGE_CHANGE
	*2012-02-02, taebum81.kim@lge.com
	*
	int rc;
	*/
...
/*
	if (IS_ERR(vreg_kt)) {
		printk(KERN_ERR"[KeyTouch] regulator_get fail : touch\n");
		return -1;
	}
*/
#ifdef CONFIG_LGE_DOMESTIC
...
#if 1 /*LGE_CHANGE_S 20120206, taebum81.kim@lge.com */
static struct regulator *vreg_ldo13 = NULL;
static struct regulator *vreg_ldo19 = NULL;

static struct regulator_bulk_data regs[2] =
{
		{ .supply = "ldo13", .min_uV = 2700000, .max_uV = 2700000 },
		{ .supply = "ldo19", .min_uV = 1800000, .max_uV = 1800000 },
};

void __init touch_power_init(void)	
{	
	int rc = 0;
	...
	vreg_ldo13 = regs[0].consumer;
	vreg_ldo19 = regs[1].consumer;
...
static int main_ts_set_vreg(unsigned char onoff)
{
    ...
    if (onoff)
    {
        rc = regulator_enable(vreg_ldo13);
        if (rc)
        {
    ...
    {
        rc = regulator_disable(vreg_ldo13);
        if (rc)
        {
            printk("%s: vreg_disable() = %d \n", __func__, rc);
        }
        rc = regulator_disable(vreg_ldo19);
        if (rc)
        {
            printk("%s: vreg_disable() = %d \n", __func__, rc);
        }
...
static struct regulator_bulk_data sensor_regulators[] = {
	{ .supply = "gp7", .min_uV = 1800000, .max_uV = 1800000 },
	{ .supply = "xo_out", .min_uV = 2600000, .max_uV = 2600000 },
};

int __init sensor_power_init(void)
{
	int rc = regulator_bulk_get(NULL, ARRAY_SIZE(sensor_regulators), sensor_regulators);
	if (rc) {
		pr_err("%s: could not get regulators: %d\n", __func__, rc);
		goto out;
	}

	rc = regulator_bulk_set_voltage(ARRAY_SIZE(sensor_regulators), sensor_regulators);
	if (rc) {
		pr_err("%s: could not set regulator voltages: %d\n",
				__func__, rc);
		goto reg_free;
...
reg_free:
	regulator_bulk_free(ARRAY_SIZE(sensor_regulators), sensor_regulators);
...
//			vreg_set_level(gp7_vreg, 1800);
			...
			regulator_bulk_enable(1, &sensor_regulators[0]);
		}
...
//			vreg_disable(gp7_vreg);
			regulator_bulk_disable(1, &sensor_regulators[0]);
		}
	}
...
//			vreg_enable(xo_out_vreg);
			regulator_bulk_enable(1, &sensor_regulators[1]);
		}
		xo_out_on_mask |= mask;
	} else
	{
...
//			vreg_disable(xo_out_vreg);
			regulator_bulk_disable(1, &sensor_regulators[1]);
		}
	}
...
{
	printk("victor_init_i2c_acceleration  %d \n",bus_num);
