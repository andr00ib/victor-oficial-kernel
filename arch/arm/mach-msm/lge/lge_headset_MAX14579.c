...
/* LGE_UPDATE_S dohwan.an@lge.com add notification */
/* Ice Cream Sandwich : is changed
 * source file is " frameworks/base/services/java/com/android/server/WiredAccessoryObserver.java "
 * HEADSET_UEVENT_MATCH = "DEVPATH=/sys/devices/virtual/switch/h2w"
 * HEADSET_STATE_PATH = /sys/class/switch/h2w/state
 * HEADSET_NAME_PATH = /sys/class/switch/h2w/name
 * INPUT = SW_HEADPHONE_INSERT , KEY_MEDIA
...
#if defined (CONFIG_LGE_DOMESTIC) || defined(LGE_MODEL_E739)
...
static void remove_headset(void)
{
	...
	if (switch_get_state(&hi->sdev) == LGE_HEADSET)	{
		...
		irq_set_irq_wake(hi->hook_switch_irq, 0);
...
static void insert_headset(bool has_hook)
{
	...
	if (has_hook)	{
		...
		irq_set_irq_wake(hi->hook_switch_irq, 1);
		local_irq_restore(irq_flags);
	} else	{
...
//	hi->btn_debounce_time = ktime_set(0, 30000000); /* 30 ms */
	...
	ret = irq_set_irq_wake(hi->hs_detect_irq, 1); /* LGE_UPDATE kideok.kim@lge.com 20111216 kernel 3.0.x Adaptation */
	if (ret < 0) {
...
/* LGE_CHANGE_S  dohwan.an@lge.com : changed this line because kernel irq unbalanced warning */
	ret = irq_set_irq_wake(hi->hook_switch_irq, 1); /* LGE_UPDATE kideok.kim@lge.com 20111216 kernel 3.0.x Adaptation */
