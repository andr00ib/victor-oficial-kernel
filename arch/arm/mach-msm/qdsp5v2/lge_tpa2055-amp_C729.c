...
char amp_cal_data[AMP_CAL_MAX] = {IN1GAIN_0DB, HPL_VOL_M6DB,                    /* HEADSET_VOICE */
                                                                  IN2GAIN_0DB, SPK_VOL_0DB,            /* SPEAKER_VOICE */
                                                                  IN1GAIN_0DB, HPL_VOL_M10DB,          /* TTY */
                                                                  IN1GAIN_0DB, HPL_VOL_0DB,          /* HEADSET_AUDIO */
                                                                  IN1GAIN_12DB, HPR_VOL_M12DB, SPK_VOL_0DB,  /* HEADSET_SPEAKER */
                                                                  IN2GAIN_6DB, SPK_VOL_M3DB};        /* SPEAKER_AUDIO */
...
void set_amp_speaker_GAN_voice(void)
{
	int fail=0;
	fail |= WriteI2C(SUBSYSTEM_CONTROL, (~SWS & ~BYPASS & ~SSM_EN));
	fail |= WriteI2C(INPUT_CONTROL, (IN2GAIN_12DB  | IN2_SE)); 							//Modify for desired IN gain 
	fail |= WriteI2C(SPEAKER_VOLUME, (SPK_EN | amp_cal_data[SPEAKER_VOICE_OUTPUT] ));							//Modify for desired SP gain
	fail |= WriteI2C(SPEAKER_OUTPUT, SPKOUT_IN2);

    MM_INFO("3 set_amp_speaker_GAN_voice() %d\n", fail);
}
EXPORT_SYMBOL(set_amp_speaker_GAN_voice);
