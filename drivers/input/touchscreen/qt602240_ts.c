...
#define QT602240_RESET_TIME		65 //100	/* msec */
...
//2011.10.05
#define QT602240_AUTOCAL_LIVE_TIME 1500 * 2  //1500msec
...
/***************************************************************************
 * Debug Definitions
 * 2012.02.27, taebum81.kim@lge.com
 ***************************************************************************/
enum {
	QT602240_TS_DEBUG_TOUCH_EVENT = 1U << 0,
	QT602240_TS_DEBUG_HAS_EARLYSUSPEND = 1U << 1,
	QT602240_TS_DEBUG_CAlIBRATE_CHIP = 1U << 2,
	QT602240_TS_DEBUG_DUMP_MESSAGE = 1U << 3,
	QT602240_TS_DEBUG_INTERRUPT = 1U << 4,
};

/* Debug Mask Initialization */
static uint32_t qt602240_debug_mask = QT602240_TS_DEBUG_HAS_EARLYSUSPEND;

/* Module Parameter Initialization */
module_param_named(debug_mask, qt602240_debug_mask, uint, S_IWUSR | S_IRUGO);

/* Debug Mask MACRO Definition */
#define ts_debug(mask, x...) \
        do { \
                if (qt602240_debug_mask & mask) \
                        printk(KERN_INFO x); \
        } while (0)
/****************************************************************************/
#ifdef CONFIG_MACH_MSM8X55_VICTOR
...
{
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "reportid:\t0x%x\n", message->reportid);
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "message1:\t0x%x\n", message->message[0]);
#ifndef CONFIG_MACH_MSM8X55_VICTOR
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "message2:\t0x%x\n", message->message[1]);
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "message3:\t0x%x\n", message->message[2]);
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "message4:\t0x%x\n", message->message[3]);
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "message5:\t0x%x\n", message->message[4]);
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "message6:\t0x%x\n", message->message[5]);
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "message7:\t0x%x\n", message->message[6]);
	ts_debug(QT602240_TS_DEBUG_DUMP_MESSAGE, "checksum:\t0x%x\n", message->checksum);
#endif
}
...
{
	ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "qt602240_calibrate_chip");
...
void qt602240_check_chip_calibration(struct qt602240_data *data)
{
	...
	if(first_good == 1)
	{
		...
		while (!((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00))) {
			...
			if (try_ctr > 10) {
				/* Failed! */
				ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[QT602240] Diagnostic Data did not update!!\n");
				qt_timer_state = 0;
				break;
			}
			msleep(2);
		...
		if ((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
			...
			ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP][SUKER] atch_ch = %d\n", atch_ch);
			ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP][SUKER] tch_ch = %d\n", tch_ch);
			ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP][SUKER] qt_time_diff = %ld\n", qt_time_diff);
			...
			if (tch_ch > 0 && tch_ch <= 15 && atch_ch == 0) 
			{
				...
				if (qt_timer_state == 1) {
					if (qt_time_diff > 500) {
						ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP]			calibration was good\n");
						if (first_good) {
							...
							qt602240_write_object(data, QT602240_TOUCH_MULTI, QT602240_TOUCH_TCHTHR, 45);  //hyuki
			...
			{
				ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP] calibration was bad1\n");
				if (atch_ch >= 100)	{
//					printk("atch_ch >= 10 \n");
...
//					if (anti_touch_30_count >= 10)
				...
				{
					ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[QT602240] calibration was not decided yet by MOVE\n");
				...
				{
//					printk("ttttttttttttttttttttttttttt \n");
			...
			{
				ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP] calibration was bad2\n");
			...
			else if (atch_ch == pre_atch_ch) 
			{
				count++;
				if (count == 5) {
					ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP] calibration was bad\n");
					/* cal was bad - must recalibrate and check afterwards */
					qt602240_calibrate_chip(data);
					qt_timer_state = 0;
					qt_time_point = jiffies_to_msecs(jiffies);
			...
			{
				ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[QT602240] calibration was not decided yet\n");
				/* we cannot confirm if good or bad - we must wait for next touch  message to confirm */
				qt602240_cal_check = 1;
				/* Reset the 100ms timer */
				qt_timer_state = 0;
	...
	if(config_check_flag){
		...
		if(qt_time_diff > QT602240_AUTOCAL_LIVE_TIME)		{
			ts_debug(QT602240_TS_DEBUG_CAlIBRATE_CHIP, "[TSP] change_auto_calibration was stopped to qt_time_diff value : %ld\n", qt_time_diff);
...
static void qt602240_input_report_release_flush(struct qt602240_data *data)
{
	...
	ts_debug(QT602240_TS_DEBUG_HAS_EARLYSUSPEND, "[TSP] === Release Event Flush\n");
...
//	mutex_lock(&input_dev->mutex);
	if (status & QT602240_RELEASE || bIsIntensiveRelease) {
		...
		ts_debug(QT602240_TS_DEBUG_TOUCH_EVENT,
				"[TSP] [%d] status & QT602240_RELEASE ----- \n",id);
...
static irqreturn_t qt602240_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		{
			ts_debug(QT602240_TS_DEBUG_INTERRUPT, "[TSP] === Touch Early Suspend\n");
...
//					printk("aaaaaaaaaaaa \n");
	...
	if(bWasSuspended == true)
	{
		ts_debug(QT602240_TS_DEBUG_INTERRUPT, "[TSP] === Touch Early Suspend 2\n");
	}
	else
	{
		if (esd_check >= 5) {
			ts_debug(QT602240_TS_DEBUG_INTERRUPT, "ESD!! Touch Reset!!!\n");
			reset_chip(data);
		}
...
//	fw_size = sizeof(QT602240_firmware);
  firmware_data = QT602240_firmware_2_0;
  fw_size = sizeof(QT602240_firmware_2_0);
#endif
...
// LGE_CHANGE_START : kenneth.kang@lge.com touch event enable
//	__set_bit(BTN_TOUCH, input_dev->keybit);
// LGE_CHANGE_END : kenneth.kang@lge.com
...
//	mutex_lock(&input_dev->mutex);
	...
	//if(config_check_flag == true){
		qt602240_write_object(data, QT602240_GEN_ACQUIRE, QT602240_ACQUIRE_TCHAUTOCAL, 0x5);
		qt602240_write_object(data, QT602240_GEN_ACQUIRE, QT602240_ACQUIRE_ATCHFRCCALTHR, 10);
		qt602240_write_object(data, QT602240_GEN_ACQUIRE, QT602240_ACQUIRE_ATCHFRCCALRATIO, 215);
		first_good = 1;
		qt_time_diff = 0;
	//}
...
static void qt602240_early_suspend(struct early_suspend *h)
{
	...
	ts_debug(QT602240_TS_DEBUG_HAS_EARLYSUSPEND, "[TSP] qt602240_early_suspend --- \n");
	bWasSuspended = true;
...
static void qt602240_late_resume(struct early_suspend *h)
{
	...
	ts_debug(QT602240_TS_DEBUG_HAS_EARLYSUSPEND, "[TSP] qt602240_late_resume --- \n");
	bWasSuspended = false;
#ifdef CONFIG_MACH_MSM8X55_VICTOR
	if(firmware_status != UPDATE_FIRM_UP)
#endif
