...
// enable / disable camera debug message
// jungsu06.kim@lge.com 
//#define LGE_ENABLE_DEBUG_MSG


#ifdef LGE_ENABLE_DEBUG_MSG
#undef CDBG
#undef pr_err
#define CDBG(fmt, args...)      printk(KERN_INFO "msm_camera: " fmt, ##args)
#define pr_err(fmt, args...) printk(KERN_INFO "msm_camera: " fmt, ##args)
#endif //LGE_ENABLE_DEBUG_MSG
	...
	case CMD_STATS_ENABLE:
		axi_data.bufnum1 =
			msm_pmem_region_lookup(&sync->pmem_stats,
              ...
              // klocwork
              if(axi_data.bufnum1 >= 8)  {
			pr_err("%s: region bufnum1 overflow error\n", __func__);
			return -EINVAL;
		}
		axi_data.bufnum2 =
			msm_pmem_region_lookup(&sync->pmem_stats,
		...
		struct msm_vfe_cfg_cmd *cfgcmd)
{
	...
	case CMD_AXI_CFG_VIDEO:
		...
		// klocwork
		if(axi_data.bufnum1 >= MAX_PMEM_CFG_BUFFERS)  {
		    pr_err("%s %d: region bufnum1 overflow error\n", __func__, __LINE__);
		    return -EINVAL;
		}
	...
	case CMD_AXI_CFG_ZSL:
		...
		if (!axi_data.bufnum2) {
			...
			return -EINVAL;
              ...
              // klocwork
              if((axi_data.bufnum1 + axi_data.bufnum2) >= MAX_PMEM_CFG_BUFFERS)  {
			pr_err("%s %d: region bufnum1 overflow error\n", __func__, __LINE__);
			return -EINVAL;
		}
