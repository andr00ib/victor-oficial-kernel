...
// enable /disable CDBG log message
...
//	printk("[QCTK_GAIN].mt9p017_write_exp_gain Func. Gain = %d, Line = %d\n", gain, line);
	...
	{
		rc = mt9p017_i2c_write_w_sensor(0x3100, 0x0002);
		//jueul.jang@lge.com 2013-06-02 : modify Noise register for noise reduction in low light
		//rc = mt9p017_i2c_write_w_sensor(0x3102, 0x70);	//112
		rc = mt9p017_i2c_write_w_sensor(0x3102, 0xD0);	//112
...
static int32_t mt9p017_sensor_setting(int update_type, int rt)
{
	...
	case REG_INIT:
		if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
			...
			/* soft reset */
			rc = mt9p017_i2c_write_b_sensor(0x0103,0x01);
			if (rc < 0)
				return rc;
			msleep(10);
...
//[LGE_UPDATE_S] jeonghoon.cho@lge.com for camera hw rev reading test
				...
				//jueul.jang@lge.com 2013-03-21 : for e730 camera hw rev
