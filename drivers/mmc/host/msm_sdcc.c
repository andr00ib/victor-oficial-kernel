...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	*/
	...
	//mmc->caps |= MMC_CAP_WAIT_WHILE_BUSY; //For Hynix eMMC slgi97.park@lge.com
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (host->mmc->index == 1)
	{
		printk(KERN_ERR "msmsdcc_runtime_suspend(%s) - Start\n", mmc_hostname(mmc));
		msmsdcc_setup_clocks(host, false);
                host->clks_on = 0;
		printk(KERN_ERR "msmsdcc_runtime_suspend() - End\n");
		return 0;
	}
#endif
	if (host->plat->is_sdio_al_client) {
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	if (host->mmc->index == 1)
	{
		printk(KERN_ERR "msmsdcc_runtime_resume(%s) - Start\n", mmc_hostname(mmc));
		...
		printk(KERN_ERR "msmsdcc_runtime_resume() - End\n");
