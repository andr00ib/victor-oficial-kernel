...
int usb_remove_device(struct usb_device *udev)
{
	...
	if(hub == NULL) //wbt error : (usb)acespirit 2013.4.19
		return -1;
...
static int find_port_owner(struct usb_device *hdev, unsigned port1,
		void ***ppowner)
{
	if (hdev == NULL) //wbt fixed : (usb)acespirit 2013.5.8
		return -1;
	...
	if(hdev_to_hub(hdev) == NULL) //wbt fixed : usb jyoung.jung 2013.05.27
		return -1;
	
	*ppowner = &(hdev_to_hub(hdev)->port_owners[port1 - 1]);
	if(*ppowner == NULL) //wbt error : (usb)acespirit 2013.4.19
		return -1;
	return 0;
}
